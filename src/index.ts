...
import { IsNullOrWhitespace, parseEnum } from "./common/string-utils";

export async function run(): Promise<void> {
  try {
    const taskConfig = new TaskConfig();
    const assertionOrchestrator : AssertionValidationOrchestrator = await initialize(taskConfig);
    const taskId: string | undefined = tl.getInputRequired("taskId");
    const expectedMessages : string[] = tl.getInput("expectedMessage", false)?.split(";") ?? [];
    
    const expectedTaskResultInput : string = tl.getInput("expectedTaskResult") 
      ?? tl.TaskResult.Succeeded.toString();

    const expectedTaskResult = parseEnum(tl.TaskResult, expectedTaskResultInput);
    ...
    if(!projectName || IsNullOrWhitespace(projectName)){
      throw new Error(`Invalid Input. Parameter: ProjectName Value: ${projectName}`);
    }
...
async function initialize(taskConfig : TaskConfig): Promise<AssertionValidationOrchestrator> {
